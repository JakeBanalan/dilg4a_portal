    ...
    methods: {
        ...
        fetchResolutionData() {
            ...
            axios.get(`../../../api/fetchResolution/8`)
                .then(response => {
                    console.log('Fetched resolution:', response.data);

                    // Make sure we have a delta array
                    if (response.data && response.data.reso_content) {
                        try {
                            this.quill.setContents({ ops: response.data.reso_content });

                            // Move cursor to end
                            // this.quill.setSelection(this.quill.getLength(), 0);
                        } catch (e) {
                            console.error("Failed to load delta into Quill:", e);
                        }
                    }
                })
                .catch(error => {
        ...
        SaveContent() {
            ...
            axios.post('../../api/PostResolution', {
                content: delta // or html, depending on what you're storing
            })
                .then(response => {
